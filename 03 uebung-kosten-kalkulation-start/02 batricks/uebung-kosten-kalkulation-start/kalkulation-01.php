...
if (isset($_POST['submit'])) {
    ...
    // auslesen und mögliche punkte durch kommas ersetzen
    $preis = $_POST['preis'];
    $anzahl = $_POST['anzahl'];
    $rabatt = $_POST['rabatt'];
...
if (isset($preis, $anzahl, $rabatt, $steuern, $raten)) {
    ...
    // Zahlen formatieren, falls ein Wert vorhanden ist.
    if (!empty($preis && $anzahl && $rabatt && $steuern && $raten)) {
        ...
        $preisf = $preis;
        $rabattf = $rabatt;
        $gesamtnetf = $gesamtnet;
        $steuernf = $steuern;
        $gesamtbruf = $gesamtbru;
        $gesamtstef = $gesamtste;
        $ratepmf = $ratepm;
    }
